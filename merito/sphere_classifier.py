...
def download_ya_disk(public_key):
    ...
    dist_path = os.path.join(my_path, 'models/resume_clf.pkl')
    with open(dist_path, 'wb') as f:
        f.write(response.content)


# Проверяем, существуют ли модели локально, если нет, скачиваем их
if not os.path.exists(os.path.join(my_path, "models/company_word2vec_russian.model")):
...
model_clf = pickle.load(open(os.path.join(my_path, "models/resume_clf.pkl"), "rb"))
...
def get_predict_for_resume(df_resume):
    ...
    """
    df_resume = deepcopy(df_resume)
    if "id" in df_resume.columns:
        df_resume.rename({"id": "id_cv"}, axis=1, inplace=True)
    ...
    if RETRAIN_MODEL:
        ...
        model_comp_name.save(os.path.join(my_path, "models/company_word2vec_russian.model"))
    else:
        # Загружаем готовую модель
        model_comp_name = Word2Vec.load(os.path.join(my_path, "models/company_word2vec_russian.model"))
    ...
    if RETRAIN_MODEL:
        ...
        model_demand.save(os.path.join(my_path, "models/demand_word2vec_russian.model"))
    else:
    ...
    if RETRAIN_MODEL:
        ...
        comp_name_vectorizer = pickle.load(open(os.path.join(my_path, "models/comp_name_vectorizer.pkl"), "rb"))
    ...
    if RETRAIN_MODEL:
        ...
        demands_vectorizer = pickle.load(open(os.path.join(my_path, "models/demands_vectorizer.pkl"), "rb"))
    ...
    if RETRAIN_MODEL:
        ...
        achiv_vectorizer = pickle.load(open(os.path.join(my_path, "models/achiv_vectorizer.pkl"), "rb"))
    ...
    return job_predict.flatten().tolist()
